...
class Ciudad {
  ...
  method defensaInicial() =  trabajadores.filter({t=>t.potencia() > potenciaMinima}).size()

  method avanzada() = (nivelTecnologico > 5 ) and ( trabajadores.all({t=>t.habilidad() > 12}))
  

  method defensaReal() = self.delegacionExperimentada().sum({t=>t.potencia()})
